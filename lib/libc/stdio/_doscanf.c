 ...
 */
/*
 * port from minix for firkeylin by Liuxiaofeng.
 */
 ...
 */
static char *o_collect(int c, FILE *stream, char type, int width, int *basep)
{
	char *bufp = inp_buf;
	int base;
	...
	while (1) {
		...
		case 'X': /* ditto */
			...
			if (!(flags & FL_NOASSIGN)) {
				if (kind == 'd' || kind == 'i')
					val = strtol(inp_buf, &tmp_string,
							base);
				else
					val = strtoul(inp_buf, &tmp_string,
							base);
				if (flags & FL_LONG)
